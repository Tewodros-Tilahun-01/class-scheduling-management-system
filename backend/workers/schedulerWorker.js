...
// Utility functions from original scheduler
...
parentPort.on("message", async (data) => {
  ...
  try {
    ...
    if (activities.length === 0) {
      throw new Error(
        "No activities found for the specified semester and user"
      );
    }

    // Update progress to 20% - Data fetched
    ...
    while (attempt < MAX_RETRIES) {
      ...
      if (conflicts.length === 0) {
        ...
        try {
          ...
          // Fetch and group the saved schedules
